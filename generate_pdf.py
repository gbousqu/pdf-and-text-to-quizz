...
def generate_questions(data, pdf: PDF, print_response:  bool = False):
    ...
    for question_data in data:
        ...
        for option in options:
            ...
            if x > 190:
                pdf.set_xy(5, y + 1)
            pdf.multi_cell(0, 5, option)

        # Add response
        response = "?"
        if print_response:
            response = question_data["reponse"]
        pdf.set_x(10)  # Move the cursor 10 units from the left margin
        pdf.cell(0, 10, f"Reponse: {response}", ln=True)
...
def generate_pdf(filename , json_data):
    ...
    pdf.add_font('DejaVu', '', 'DejaVuSansCondensed.ttf', uni=True) # Add a Unicode font (uses UTF-8) ajout bousquet
    pdf.set_font("DejaVu", "", 8)
